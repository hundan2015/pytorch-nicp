...
def non_rigid_icp_mesh2mesh(
    ...
    return non_rigid_icp_mesh2pcl(
        ...
        device,
        target_lm=target_lm,
        point_landmark_data=point_landmark_data,
    )
...
def non_rigid_icp_mesh2pcl(
    ...
    boundary_mask = mesh_boundary(
        template_mesh.faces_padded()[0], template_vertex.shape[1]
    )
    ...
    template_lm = template_lm.unsqueeze(0)
    target_lm = target_lm.float()
    R, T, s = corresponding_points_alignment(
        template_lm, target_lm, estimate_scale=True
    )
    transformed_vertex = (
        s[:, None, None] * torch.bmm(template_vertex, R) + T[:, None, :]
    )
    ...
    if in_affine is None:
        local_affine_model = LocalAffine(
            template_vertex.shape[1], template_vertex.shape[0], template_edges
        ).to(device)
    else:
    ...
    for i in loop:
        ...
        for _ in inner_loop:
            ...
            landmark_sum = (
                torch.sum(landmark_distance) * landmark_weights[w_idx] / bsize
            )
            print(landmark_sum)
